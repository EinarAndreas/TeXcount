...
Applicable Unicode properties/scripts include \code{Digit}, \code{Latin}, \code{Greek}, \code{Cyrillic}, \code{Hebrew}, \code{Arabic}, \code{Han}, \code{Katakana}, \code{Hiragana}, and more.\footnote{A more complete overview is available at Wikipedia: \url{https://en.wikipedia.org/wiki/Script_(Unicode)}.}
...
\item[Preamble]A few macros should be counted even if they are in the preamble. In particular, \code{\bs{title}\{\textit{title text}\}} is counted as a header assuming it will later be used to produce a title.
...
\item[package include]When packages are included using \code{\bs{usepackage}\{\textit{name}\}}, \TeXcount{} will check for package specific macro handling rules to include. Initially, only \code{\bs{usepackage}} triggers package inclusion, but more macros may be added to the \code{\%TeXpackageinc} hash.
...
The template is a string with codes for inserting the count values and titles. To specify it, you can use the option \code{-template="\textit{template}"}. The encapsulating \code{"\ldots"} are required if the template contains spaces. You may insert line shifts by using \code{\bs{n}}.

Alternatively, you can specify tempates within the document itself using the \TeXcount{} instructions \code{\%TC:newtemplate} followed by one or more \code{\%TC:template \textit{template-text}} lines.

The counts may be included by using the counter keywords---\code{word}, \code{headerword}, etc---or arithmetic combinations of these, eg \code{word+otherword+2*headerword}. Other codes that may be inserted are: \code{\{SUM\}} to insert the count as specified by the \code{-sum} option, \code{\{TITLE\}} for the title (e.g. section name) and a header (same as title unless \TeXcount{} has replaced it), \code{\{ERROR\}} for the number of parsing errors, \code{\{WARNINGS\}} for the number of distinct warnings, or \code{\{NWARNINGS\}} for the total number of warnings. Some of these also have shortened forms\footnote{Previously, one-letter versions of some of these codes were permitted, but that is no longer the case.} like \code{\{ERR\}} and \code{\{WARN\}}.
...
If all you want is to replace missing or null values with an alternative text, you can use \code{\{\textit{label}|\textit{null-text}\}}, which will give the value of the counter if non-zero, otherwise the alternative null-text.
...
The formerly used numberic codes are listed at the end of the keyword list for each state. Before version 3, these numeric codes were used to specify parsing rules, but although these should still work, using the named keywords is highly recommended.
...
\subsection{Counting numbers separately}

By default, \TeXcount{} has a set of characters allowed within words, and identifies words as any sequence of
these characters. As a consequence, it does not distinguish between words composed entirely of letters, numbers, and
words containing a mix of letters and digits.

It is possible\footnote{The ability to count different types of words separately was introduces in \TeXcount{} version 3.2.}
to specify \code{wordtype} rules that changes how words of type \code{number}, \code{mixed}, and \code{nonum} (pure words with no digits) are counted. This is done by adding \TeXcount{} instructions of the following format:
\codeline{\%TC:wordtype \textit{original-rule} \textit{wordtype} \textit{new-rule}}
The rules will usually just correspond to the original counter, so in order to specify that a number originally counted as a word within the main text should be counted using a separate counter, you may add the following code:
%
\begin{lstlisting}
%TC:newcounter textnumber Numbers in text
%TC:wordtype word number textnumber
\end{lstlisting}
%
Whenever \TeXcount{} encounters a word parsed as \code{word} (alias of \code{text}), it checks if it matches the pattern for word type \code{number}, and if it does changes the counting rule to \code{textnumber}.

\textit{NB: Note that this has been added as an experimental feature, and may change in future versions of \TeXcount{}.}
...
\codeline{\%TC:fileinclude \parm{macro} \parm{file-parameters}}
...
\codeline{\url{https://www.latex-project.org/lppl.txt}}
